...
public class CountNoOfMaximumBitwiseORSubSets {
    ...
    private void backtrack(int[] nums, int idx, int currOr) {
        if (idx == nums.length) {
        ...
        // Include nums[idx]
        backtrack(nums, idx + 1, currOr | nums[idx]);
        // Exclude nums[idx]
        backtrack(nums, idx + 1, currOr);
    }

    public static void main(String[] args) {
        CountNoOfMaximumBitwiseORSubSets solution = new CountNoOfMaximumBitwiseORSubSets();
        int[] nums1 = {3, 1};
