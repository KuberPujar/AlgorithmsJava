 ...
 */
public class MaxValueOfEquation_SW {
    public int findMaxValueOfEquation(int[][] points, int k) {
        Deque<Integer> deque = new ArrayDeque<>();
        int maxValue = Integer.MIN_VALUE;

        for (int j = 0; j < points.length; j++) {
            // Remove points that are outside the window (xi < xj - k)
            while (!deque.isEmpty() && points[deque.peekFirst()][0] < points[j][0] - k) {
                deque.pollFirst();
            }

            // Calculate the equation value for valid points
            if (!deque.isEmpty()) {
                int i = deque.peekFirst();
                int currentValue = points[i][1] + points[j][1] + points[j][0] - points[i][0];
    ...
    public static void main(String[] args) {
        MaxValueOfEquation_SW solution = new MaxValueOfEquation_SW();
